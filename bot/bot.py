...
init_extensions = ['cogs.owner',
                   'cogs.info',
                   'cogs.counting'
                   ]

help_cmd_attrs = {'name': 'help',
...
class Bot(AutoShardedBot):
    ...
    async def ready(self):
        ...
        await self.change_presence(status=discord.Status.online, activity=discord.Game("Mole World"))
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.NoPrivateMessage):
            await ctx.author.send('You can not use this command in DMs.')
        elif isinstance(error, commands.DisabledCommand):
            await ctx.author.send('Command is disabled.')
